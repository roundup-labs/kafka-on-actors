 ...
 *        load balanced over all servers irrespective of which servers are specified here for bootstrapping—this list
...
case class ScalaKafkaProducer(config: ProducerConfig) extends Logging with Callback {
    ...
    def send(topic: String, message: Array[Byte]): Unit = {
        logger.debug(s"ScalaKafkaProducer.send($topic)")
        producer.send(new ProducerRecord(topic, message), this)
    }
